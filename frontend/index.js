...
class FitApp {
  ...
  bindUI() {
    // header
    qs('#btn-refresh').addEventListener('click', () => this.refreshResults());
    qs('#btn-cabinet').addEventListener('click', () => this.showCabinet());
...
// mobile bottom nav (если есть)
    ...
    // cabinet
    qs('#btn-back-main').addEventListener('click', () => this.showMain());
    qs('#btn-update-db').addEventListener('click', () => this.updateDB());
    ...
    qs('#btn-form-cancel').addEventListener('click', () => this.resetProfileForm());

    // admin
    qs('#btn-back-from-admin').addEventListener('click', () => this.showMain());
    qs('#btn-admin-refresh').addEventListener('click', () => this.refreshAdmin());
    qs('#btn-admin-download').addEventListener('click', () => this.downloadAdminJSON());
  ...
  async loadProfilesAndRender() {
    ...
    const emptyEl = qs('#profiles-empty');
    listEl.innerHTML = '';
    ...
    try {
      ...
      emptyEl.innerText = 'Не удалось загрузить профили (проверь, запущен ли backend).';
      return;
    }

    if (!profiles.length) {
      emptyEl.classList.remove('hidden');
      emptyEl.innerText = 'Профилей пока нет — добавь новый ниже.';
      return;
    }
  ...
  resetProfileForm() {
    qs('#profile-form-title').textContent = 'Новый профиль';
    qs('#profile-id').value = '';
    qs('#profile-name').value = '';
    qs('#profile-gender').value = 'male';
    ['height','chest','shoulders','waist','hips','arm_length','leg_length'].forEach((k) => {
      const el = qs(`#profile-${k}`);
      if (el) el.value = '';
    });
  }
  ...
  async loadProfileIntoForm(id) {
    ...
    qs('#profile-form-title').textContent = 'Редактирование профиля';
    qs('#profile-id').value = String(p.id);
    qs('#profile-name').value = p.name ?? '';
    qs('#profile-gender').value = p.gender ?? 'male';
    ['height','chest','shoulders','waist','hips','arm_length','leg_length'].forEach((k) => {
      const el = qs(`#profile-${k}`);
      if (el) el.value = p[k] ?? '';
    });
    ...
    // scroll to form
    qs('#profile-form').scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  async saveProfileFromForm() {
    const id = qs('#profile-id').value.trim();
    const payload = {
      name: qs('#profile-name').value.trim(),
      gender: qs('#profile-gender').value,
      height: safeNum(qs('#profile-height').value),
      chest: safeNum(qs('#profile-chest').value),
      shoulders: safeNum(qs('#profile-shoulders').value),
      waist: safeNum(qs('#profile-waist').value),
      hips: safeNum(qs('#profile-hips').value),
      arm_length: safeNum(qs('#profile-arm_length').value),
      leg_length: safeNum(qs('#profile-leg_length').value),
    };
